...
public sealed interface PublishOutcome {
    ...
    static PublishOutcome from(OutboxMessage message, Throwable error) {
        ...
        return switch (error) {
            case java.util.concurrent.TimeoutException t -> new Retryable(message.id(), "broker timeout", t);
            case InterruptedException i -> new Retryable(message.id(), "interrupted", i);
            case IllegalArgumentException e -> new Fatal(message.id(), "invalid message: " + e.getMessage(), e);
            default -> new Retryable(message.id(), error.getClass().getSimpleName(), error);
        };
    }
